...
class Translator(nn.Module):
    def __init__(self, latent_size, device, num_embeddings, embedding_dim):
        ...
        self.fc = nn.Sequential(
            nn.Linear(2*latent_size, latent_size * 4),
            nn.LeakyReLU(0.2),
...
class Generator(nn.Module):
  ...
  def forward(self, z, y, return_emb=False):
    ...
    translator_emb = self.translator(z, y)
    
    # First linear layer
    h = self.linear(translator_emb)
    # Reshape
...
class G_D(nn.Module):
  ...
  def forward(self, z, gy, x=None, dy=None, train_G=False, return_G_z=False,
    ...
    with torch.set_grad_enabled(train_G):
      ...
      G_z = self.G(z, self.G.shared(gy)) # why generator outputs 128x128 images
      # Cast as necessary

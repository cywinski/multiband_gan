...
class Validator:
    ...
    #                 self.score_model_func(generations[start_point:end_point]).cpu().detach().numpy().reshape(-1,
    #                                                                                                          self.dims))
    ...
    #             ref_data=distribution_gen[np.random.choice(len(distribution_gen), len(distribution_orig), True)]
...
#             self, curr_global_decoder, class_table, task_id, translate_noise=True, starting_point=None,

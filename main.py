    ...
    for task_id in tasks_to_learn:
        ...
        if args.skip_validation:
            ...
            if (args.training_procedure == "multiband") and (
                ...
                wandb.log({f"local_gan_FID/task_{task_id}": fid_result})
                if len(generated_classes):
                    wandb.log(
                        {
                            f"local_gan_generated_classes/task_{task_id}": wandb.Histogram(
            ...
            for j in range(task_id + 1):
                ...
                wandb.log({f"global_gan_FID/task_{j}": fid_result})
                if len(generated_classes):
                    wandb.log(
                        {
                            f"global_gan_generated_classes/task_{j}": wandb.Histogram(
                                generated_classes, num_bins=num_classes
                            )
                        }
                    )
                ...
                wandb.log(
                    {
                        f"final_generations/task_{j}": wandb.Image(generations),
                    }
                )
